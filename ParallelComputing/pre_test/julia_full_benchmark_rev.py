...
# =========================================================
# [1/5] CPU VECTORIZED (NumPy) - Rank 0 Only
# =========================================================
cpu_time = None
if rank == 0:
    # ข้าม NumPy หากขนาด Grid > 3000 เพื่อป้องกัน 1-Core CPU ค้าง
    if SIZE > 3000:
        print(f"\n[1/5] Skipping NumPy CPU Vectorized (Grid {WIDTH}x{HEIGHT} is too large for 1 Core)...")
    else:
        ...
        # ป้องกัน Overflow Warning ด้วย np.errstate
...
# =========================================================
# [2/5] CPU MULTI-CORE PARALLEL (Numba JIT) - Rank 0 Only
# =========================================================
...
numba_time = 0.0
if rank == 0:
    print("\n[2/5] Computing on CPU Parallel (Numba Multi-Core JIT)...")
...
# =========================================================
# [3/5] SINGLE GPU EXECUTION (OpenCL Full Grid) - Rank 0 Only
# =========================================================
gpu_time = 0.0
gpu_success = False
if rank == 0:
...
# สร้าง Sub-group ดึงมาใช้เฉพาะ 4 Ranks (Rank 0, 1, 2, 3)
target_ranks = 4
...
if rank == 0:
    ...
    # กำหนด Baseline ชัดเจน (ใช้ NumPy หากมี ถ้าไม่มีใช้ Numba)
    base_time = cpu_time if cpu_time is not None else numba_time
    base_label = "NumPy CPU" if cpu_time is not None else "Numba CPU"

    if cpu_time is not None:
        print(f" 1. NumPy CPU (1 Core)          : {cpu_time:.4f} s  (1.00x Baseline)")
    else:
        print(f" 1. NumPy CPU (1 Core)          : SKIPPED (>3000 Grid)")

    print(f" 2. Numba CPU (Multi-Core)      : {numba_time:.4f} s  ({base_time/numba_time:.2f}x vs {base_label})")
    if gpu_success:
        print(f" 3. Single GPU OpenCL           : {gpu_time:.4f} s  ({base_time/gpu_time:.2f}x vs {base_label})")
    print(f" 4. MPI ({size} Chunks) + GPU       : {mpi5_time:.4f} s  ({base_time/mpi5_time:.2f}x vs {base_label})")
    print(f" 5. MPI (4 Cores CFD Halo) + GPU: {mpi4_time:.4f} s  ({base_time/mpi4_time:.2f}x vs {base_label})")
    print("=" * 70)

    # Save output plot
    plt.figure(figsize=(10, 8))
    plot_data = mpi5_global if mpi5_global is not None else numba_output
    plt.imshow(plot_data.reshape((HEIGHT, WIDTH)), cmap='twilight_shifted', extent=[-1.5, 1.5, -1.5, 1.5])
    plt.title(f"All-in-One Julia Set Benchmark ({WIDTH}x{HEIGHT})")
    plt.colorbar(label='Iteration Count')
    plt.savefig("julia_all_benchmark.png", dpi=300)
    print("\n[Success] Saved benchmark plot to 'julia_all_benchmark.png'\n")
