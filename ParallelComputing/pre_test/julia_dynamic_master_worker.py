...
import matplotlib
matplotlib.use('Agg')  # บังคับใช้ Non-interactive backend
...
if rank == 0:
    print("=" * 70)
    print(f" 🚀 DYNAMIC MASTER-WORKER BENCHMARK ({WIDTH}x{HEIGHT})")
    print(f" Total MPI Ranks : {size} (Rank 0 = Master, Ranks 1..{size-1} = Workers)")
    print("=" * 70)

    # 1. Numba Multi-Core
    print("\n[1/3] Computing Numba CPU Multi-Core Baseline...")
    ...
    # 2. Single OpenCL GPU
    print("\n[2/3] Computing Single OpenCL GPU Baseline...")
    try:
...
if rank == 0:
    ...
    # 2. วนลูปรับผลลัพธ์และป้อนงานใหม่
    status = MPI.Status()
    ...
    # -----------------------------------------------------
    # WORKER LOGIC (Ranks 1..N) - Memory Optimized
    # -----------------------------------------------------
    platforms = cl.get_platforms()
    ctx = cl.Context([platforms[0].get_devices()[0]])
    queue = cl.CommandQueue(ctx)
    ...
    while True:
        ...
        queue.finish()
        res_buf.release()  # ปล่อย GPU Buffer ทันที

        comm.send((x, y, w, h, tile_res), dest=0, tag=TAG_RESULT)

# =========================================================
# --- [3] SUMMARY & VISUALIZATION (Rank 0 Only) ---
# =========================================================
if rank == 0:
    print("\n" + "=" * 70)
    print(f" 📊 FINAL COMPARISON SUMMARY ({WIDTH}x{HEIGHT})")
    print("=" * 70)
    print(f" 1. Numba CPU (Multi-Core)      : {numba_time:.4f} s  (1.00x Baseline)")
    print(f" 2. Single GPU OpenCL           : {gpu_time:.4f} s  ({numba_time/gpu_time:.2f}x vs Numba)")
    print(f" 3. Dynamic Master-Worker Queue : {dynamic_time:.4f} s  ({numba_time/dynamic_time:.2f}x vs Numba)")
    if gpu_time > 0:
        print(f"    --> Dynamic vs Single GPU   : {gpu_time/dynamic_time:.2f}x Speedup")
    print("=" * 70)

    # 📈 SAVE GRAPH & RESULT IMAGE
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))

    # 1. Bar Chart
    labels = ['Numba CPU', 'Single GPU', f'Dynamic Queue\n({size} Ranks)']
    times = [numba_time, gpu_time, dynamic_time]
    colors = ['#4c72b0', '#55a868', '#c44e52']

    bars = ax1.bar(labels, times, color=colors, width=0.4)
    ax1.set_ylabel('Execution Time (s)')
    ax1.set_title(f'Performance Comparison ({WIDTH}x{HEIGHT})')
    ax1.grid(axis='y', linestyle='--', alpha=0.7)

    for bar in bars:
        h = bar.get_height()
        ax1.annotate(f'{h:.3f}s', xy=(bar.get_x() + bar.get_width() / 2, h),
                    xytext=(0, 3), textcoords="offset points", ha='center', va='bottom')

    # 2. Render Julia Set
    im = ax2.imshow(final_image, cmap='twilight_shifted', extent=[-1.5, 1.5, -1.5, 1.5])
    ax2.set_title('Julia Set Result')
    fig.colorbar(im, ax=ax2)

    plt.tight_layout()
    output_png = "julia_dynamic_benchmark_graph.png"
    plt.savefig(output_png, dpi=120)  # ลด DPI ลงเล็กน้อยเพื่อประหยัด RAM
    plt.close('all')
    print(f"\n[Success] Saved graph & image to '{output_png}'\n")
